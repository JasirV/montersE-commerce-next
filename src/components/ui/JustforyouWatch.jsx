...
const StockStatus = ({ stockQuantity, inStock }) => {
  if (!inStock || stockQuantity === 0)
    return <div className="mt-2 text-xs text-red-500 bg-red-50 px-2 py-1 rounded-full">Out of Stock</div>;
  if (stockQuantity <= 10)
    return <div className="mt-2 text-xs text-orange-500 bg-orange-50 px-2 py-1 rounded-full">Only {stockQuantity} left</div>;
  return <div className="mt-2 text-xs text-green-500 bg-green-50 px-2 py-1 rounded-full">In Stock</div>;
};
    ...
    <div className="flex flex-col items-center space-y-1 w-full">
      {hasDiscount ? (
        <div className="flex flex-col xs:flex-row items-center justify-center gap-1 xs:gap-2 w-full">
          <span className="text-base xs:text-lg font-bold text-[#1e518e] whitespace-nowrap">{parseFloat(salePrice).toFixed(1)} AED</span>
          <span className="text-xs xs:text-sm line-through text-gray-400 whitespace-nowrap">{parseFloat(regularPrice).toFixed(1)} AED</span>
        </div>
      ) : (
        <span className="text-base xs:text-lg font-bold text-[#1e518e] whitespace-nowrap">{parseFloat(regularPrice || salePrice || 0).toFixed(1)} AED</span>
      )}
    </div>
  );
};
...
const ProductItem = ({ product, isInWishlist, onToggleWishlist }) => {
  ...
  const handleWishlistToggle = (e) => {
    e.stopPropagation();
    onToggleWishlist(product._id || product.sku || product.name);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 group cursor-pointer h-full flex flex-col">
      <div className="p-3 xs:p-4 flex flex-col items-center text-center relative flex-1">
        <DiscountBadge regularPrice={product?.regularPrice} salePrice={product?.salePrice} />
        <button
        ...
        >
          {isInWishlist ? <FaHeart className="text-red-500 text-sm" /> : <FaRegHeart className="text-gray-600 hover:text-red-500 text-sm transition-colors" />}
        </button>
        <div className="relative mb-3 xs:mb-4 flex-shrink-0">
          {!imageLoaded && !imageError && <div className="w-[120px] h-[120px] xs:w-[140px] xs:h-[140px] sm:w-[160px] sm:h-[160px] md:w-[180px] md:h-[180px] bg-gray-200 rounded-lg animate-pulse" />}
          <Image
            src={imageError ? "/placeholder-image.jpg" : getMainImage()}
            alt={product?.name || "Product image"}
            width={180}
            height={180}
            className={`rounded-lg transition-opacity duration-300 ${imageLoaded ? "opacity-100" : "opacity-0"} w-[120px] h-[120px] xs:w-[140px] xs:h-[140px] sm:w-[160px] sm:h-[160px] md:w-[180px] md:h-[180px] object-cover group-hover:scale-105 transition-transform duration-300`}
            loading="lazy"
        ...
        <h3 className="text-xs xs:text-sm font-semibold text-gray-800 line-clamp-2 leading-tight mb-3 group-hover:text-[#1e518e] transition-colors duration-200 flex-1" style={{ minHeight: "2.5rem" }}>
          {product?.name}
        </h3>
        ...
        <div className="flex flex-wrap justify-center gap-1 mt-2 pt-2 border-t border-gray-100 w-full">
          {product?.condition && <span className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-full">{product.condition}</span>}
        </div>
      </div>
    </div>
  );
...
const JustforyouWatch = () => {
  ...
  const [apiData, setApiData] = useState({ newArrivals: [], montresTrusted: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await axios.get( `${process.env.NEXT_PUBLIC_BASEURL}/home/trusted`);
  ...
  const generateKey = (product, index) => `${product._id || product.sku || product.name}-${index}`;
      ...
      <div className="max-w-[1536px] mx-auto">
        <div className="text-center mb-6 xs:mb-8 sm:mb-10">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-xs xs:text-sm sm:text-base px-2">
            Discover our exquisite collection of premium watches and accessories, featuring the finest craftsmanship and timeless designs.
          </p>
        </div>

        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-3 xs:px-4 py-3 rounded-lg mb-6 text-center mx-2">
            <p className="font-semibold text-sm xs:text-base">Oops! Something went wrong</p>
        ...
        <div className="flex flex-col lg:flex-row gap-4 xs:gap-5 sm:gap-6 lg:gap-8">
          ...
          <div className="bg-white rounded-xl xs:rounded-2xl p-4 xs:p-5 sm:p-6 lg:p-8 shadow-lg w-full lg:w-1/2 border border-gray-200 hover:shadow-xl transition-shadow duration-300">
            <div className="flex items-center justify-between mb-4 xs:mb-5 sm:mb-6 pb-3 xs:pb-4 border-b border-gray-200">
              <h2 className="text-lg xs:text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">New Arrivals</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-4 sm:gap-5">
              {loading
                ...
                : apiData.newArrivals.map((product, index) => (
                    <ProductItem
                      key={generateKey(product, index)}
                      product={product}
                      isInWishlist={isInWishlist(product._id || product.sku || product.name)}
            ...
            {!loading && apiData.newArrivals.length === 0 && (
              <div className="text-center py-8 xs:py-10">
                <div className="text-gray-400 text-4xl xs:text-5xl sm:text-6xl mb-3 xs:mb-4">⌚</div>
                <p className="text-gray-500 text-base xs:text-lg">No new arrivals at the moment</p>
                <p className="text-gray-400 text-xs xs:text-sm mt-1 xs:mt-2">Check back later for new products</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl xs:rounded-2xl p-4 xs:p-5 sm:p-6 lg:p-8 shadow-lg w-full lg:w-1/2 border border-gray-200 hover:shadow-xl transition-shadow duration-300">
            <div className="flex items-center justify-between mb-4 xs:mb-5 sm:mb-6 pb-3 xs:pb-4 border-b border-gray-200">
              <h2 className="text-lg xs:text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Montres Trusted</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-4 sm:gap-5">
              {loading
                ? skeletonArray.map((_, i) => <ProductSkeleton key={i} />)
                : apiData.montresTrusted.map((product, index) => (
                    <ProductItem
                      key={generateKey(product, index)}
                      product={product}
                      isInWishlist={isInWishlist(product._id || product.sku || product.name)}
            ...
            {!loading && apiData.montresTrusted.length === 0 && (
              <div className="text-center py-8 xs:py-10">
                <div className="text-gray-400 text-4xl xs:text-5xl sm:text-6xl mb-3 xs:mb-4">⭐</div>
                <p className="text-gray-500 text-base xs:text-lg">No trusted products available</p>
                <p className="text-gray-400 text-xs xs:text-sm mt-1 xs:mt-2">We're updating our trusted collection</p>
              </div>
            )}
          </div>
        </div>
        ...
        {wishlist.size > 0 && (
          <div className="fixed bottom-4 xs:bottom-6 right-4 xs:right-6 bg-gradient-to-r from-[#1e518e] to-[#0061b0] text-white px-4 xs:px-5 py-2 xs:py-3 rounded-full shadow-2xl z-50 flex items-center space-x-2 xs:space-x-3 hover:scale-105 transition-transform duration-200 cursor-pointer">
            ...
            <span className="text-xs xs:text-sm font-bold">{wishlist.size}</span>
