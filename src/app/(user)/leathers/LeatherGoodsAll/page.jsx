...
import LeatherGoodsFilterSidebar from "@/features/product/LeatherGoodsFilterSidebar";
...
const PageContent = () => {
  ...
  // Active filters structure for leather goods
  ...
  const brands = useMemo(() => {
    ...
    return productBrands.length > 0
      ...
      : ["Gucci", "Prada", "Louis Vuitton", "Hermès", "Coach", "Michael Kors"];
  }, [products.products]);

  const categories = useMemo(() => {
    const productCategories = products.products.flatMap((p) => p.categories || []).filter(Boolean);
    return productCategories.length > 0
      ? [...new Set(productCategories)]
      : ["Bags", "Wallets", "Belts", "Accessories", "Luggage"];
  }, [products.products]);
  ...
  const colors = useMemo(() => {
    ...
    return productColors.length > 0
      ...
      : ["Black", "Brown", "Tan", "Burgundy", "Navy", "Cognac"];
  }, [products.products]);

  const materials = useMemo(() => {
    const productMaterials = products.products.map((p) => p.material).filter(Boolean);
    return productMaterials.length > 0
      ? [...new Set(productMaterials)]
      : ["Full Grain Leather", "Top Grain Leather", "Genuine Leather", "Saffiano Leather", "Suede", "Exotic Leather"];
  }, [products.products]);

  const sizes = useMemo(() => {
    const productSizes = products.products.map((p) => p.size).filter(Boolean);
  ...
  const buildApiParams = useCallback(() => {
    const params = {
      ...
      leatherType: "all" // For all leather goods
    };
    ...
    console.log("Final API params for leather goods:", params);
    return params;
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      console.log("Fetching leather goods with params:", apiParams);

      const result = await LeatherBycategory("All", apiParams);

      if (result.error) {
        throw new Error(result.error.message || "Failed to fetch leather goods");
      }

      if (result.data) {
        console.log(
          "Leather goods fetched successfully:",
      ...
      setError("Failed to load leather goods. Please try again later.");
    } finally {
      setLoading(false);
        ...
        <nav className="flex mb-4 xs:mb-5 sm:mb-6" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-2 text-sm xs:text-base">
            ...
            <li className="flex items-center">
              ...
              <span className="text-gray-700 capitalize">All Leather Goods</span>
            </li>
            {category && (
              <li className="flex items-center">
                <span className="mx-2 text-gray-400">/</span>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 xs:gap-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
                    ...
                    <p className="text-sm xs:text-base font-semibold text-gray-700 bg-gray-50 px-3 py-1 rounded-lg">
                      {products.totalProducts || 0}{" "}
                      {products.totalProducts === 1 ? "item" : "items"} found
                    </p>
                  </div>
              ...
              !error && (
                <div className="text-center py-12 xs:py-16 sm:py-20 bg-white rounded-xl shadow-sm border border-gray-100">
                  <div className="max-w-md mx-auto">
                    ...
                    <h3 className="text-xl xs:text-2xl font-bold text-gray-900 mb-2">
                      No leather goods found
                    </h3>
                    <p className="text-gray-600 mb-6 text-sm xs:text-base">
                      Try adjusting your search or filter criteria to find premium leather goods.
                    </p>
                    <button
                      onClick={clearAllFilters}
                      className="px-6 py-3 text-sm xs:text-base font-semibold rounded-lg bg-[#8b6b4a] text-white hover:bg-[#6a4f36] transition-colors duration-200 shadow-sm hover:shadow-md"
      ...
      fallback={
        <div className="bg-[#f8f5f2] min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading leather goods...</p>
          </div>
        </div>
      }
    >
